...
EnemyChat5::EnemyChat5() {}
EnemyChat5::~EnemyChat5() {}

void EnemyChat5::Initialize() {
...
void EnemyChat5::Update(char* keys, char* oldkeys) {
...
void EnemyChat5::Draw() {
    ...
    if (chatReceponFlag[1] == TRUE) {
        ...
        DrawFormatString(enemyPosX[1] + 50, enemyPosY[1] + 50 + ChatScrollY, GetColor(0, 0, 0), "Q–V‚µ‚¿‚á‚Á‚½");
    }

    if (chatReceponFlag[2] == TRUE) {
        DrawGraph(enemyPosX[2], enemyPosY[2] + ChatScrollY, chatReceponGraph, TRUE);
        DrawFormatString(enemyPosX[2] + 50, enemyPosY[2] + 50 + ChatScrollY, GetColor(0, 0, 0), "‚à‚¤‹x‚à‚¤‚©‚È");
    }

    if (chatReceponFlag[3] == TRUE) {
        DrawGraph(enemyPosX[3], enemyPosY[3] + ChatScrollY, chatReceponGraph, TRUE);
        DrawFormatString(enemyPosX[3] + 50, enemyPosY[3] + 50 + ChatScrollY, GetColor(0, 0, 0), "‚ß‚ñ‚Ç‚­‚³‚¢‚æ`");
    }

    if (chatReceponFlag[4] == TRUE) {
        DrawGraph(enemyPosX[4], enemyPosY[4] + ChatScrollY, chatReceponGraph, TRUE);
        DrawFormatString(enemyPosX[4] + 50, enemyPosY[4] + 50 + ChatScrollY, GetColor(0, 0, 0), "‚»‚ñ‚È‚É‹x‚ñ‚¶‚á\n‚¾‚ßH");
    }
